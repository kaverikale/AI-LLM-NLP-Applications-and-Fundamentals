...
st.caption("Week 2 build: Planner + Tools + Memory, wired together with LangGraph.")

with st.sidebar:
    st.markdown("### Try asking:")
    st.markdown("""
- "What's the status of order **#A1029**?"
- "Can I return order **#A1031**?"
- "Track my order, ID is **#A1031**" — then follow up with *"can I return it?"* with no ID repeated
    """)
    st.markdown("---")
    st.markdown(
        "Unlike Week 1, order questions now hit a real (mock) database "
        "through a tool call — watch the 'Tool activity' note under each "
        "answer. The agent also remembers the last order ID you mentioned "
